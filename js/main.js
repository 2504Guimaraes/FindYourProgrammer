...
const montarDivNaTela = (objInfo, divRepositorios) => {
    
    const divCaixa = document.createElement('div')
    divCaixa.setAttribute('class', 'caixa-resposta-busca')
        
    const divH4 = document.createElement('h4')
    
    const linkRepo = document.createElement('a')
    linkRepo.setAttribute('href', `${objInfo.url}`)
    linkRepo.setAttribute('target', '_blank')
    linkRepo.innerText = objInfo.nome
    divH4.appendChild(linkRepo)
    divCaixa.appendChild(divH4)

    verSeTemTxtOuLinguagem(
        objInfo.descricao, 
        objInfo.linguagem,
        divCaixa
    )
    divRepositorios.appendChild(divCaixa)
}

const prepararDivsRepositorios = (listaRepos) => { 
    const divRepositorios = document.getElementById('div-repositorios') 
    for (let repo of listaRepos) {    
        const objInfo = {}
        for (info in repo) {
            info === 'nome' ? objInfo.nome = repo[info] : null
            info === 'url' ? objInfo.url = repo[info] : null
            info === 'descricao' ? objInfo.descricao = repo[info] : null
            info === 'linguagem' ? objInfo.linguagem = repo[info] : null
        }
        montarDivNaTela(objInfo, divRepositorios)
    }
}

const pegarDadosRepositorio = (repositorio) => {
    let repo = {}
    for (categoria in repositorio) {
        
        categoria === 'name' ? repo.nome = repositorio[categoria] : null
        categoria === 'svn_url' ? repo.url = repositorio[categoria] : null
        
        if (categoria === 'description') {
            repositorio[categoria] != null ? 
                repo.descricao = repositorio[categoria] : null
        }
        else if (categoria === 'language') {
            repositorio[categoria] != null ? 
                repo.linguagem = repositorio[categoria] : null
       }
    }
    return repo
}

const mostrarRepositorios = (repositorios) => {
    let listaRepositorios = []
    for (let repositorio in repositorios) {
        const infoRepositorio = pegarDadosRepositorio(repositorios[repositorio])
        listaRepositorios.push(infoRepositorio)
    }
    prepararDivsRepositorios(listaRepositorios)
}

const buscarDadosProgramador = () => {
    const inputPesquisa = document.getElementById('input-busca')
    const usuarioDigitado = inputPesquisa.value;
    const divRepositorios = document.getElementById('div-repositorios')
...
const buscarRepositorios = () => {
    let btnBuscar = document.getElementById('btn-buscar-info')
    btnBuscar.addEventListener('click', () => {
        buscarDadosProgramador()
    })
}
